...
class Gexf:
    ...
    def addGraph(self, type, mode, timestamp, timerepresentation="timestamp"):
        g = Graph(type, mode, timerepresentation, timestamp)
...
class Graph:
    ...
    def __init__(self, type, mode, timestamp, timerepresentation, start="", end=""):
        self.timestamp = timestamp
        # control variable
        self.authorizedType = ("directed", "undirected")
        self.authorizedMode = ("dynamic", "slice")
        # time format
        # Discrete: integer or double
        # Continuous : date (yyyy-mm-dd) or dateTime
        # default : double
        self.authorizedtimestamp = ("integer", "double", "date", "dateTime")

        self.defaulttimestamp = "double"
        self.defaultType = "directed"
        self.defaultMode = "slice"
    ...
    def addEdge(self, id, source, target, weight="", start="", end="", timerepresentation="", r="", g="", b="", spells=[], startopen=False, endopen=False):
        self._edges[str(id)] = Edge(self, id, source, target, weight, start, end, timerepresentation, r, g, b, spells, startopen, endopen)
        return self._edges[str(id)]

    def addNodeAttribute(self, title, defaultValue=None, type="integer", mode="slice", force_id=""):
    ...
    def checkNodeAttribute(self, id, value, start, end):
        ...
        #             if self._nodesAttributes[id]["mode"]=="slice" and ( not start=="" or not end=="") :
        #                 raise Exception("attribute "+str(id)+" is slice you can't specify start or end dates. Declare Attribute as dynamic")
        #             return 1
        #         else :
        #             raise Exception("attribute id unknown. Add Attribute to graph first")

    def addEdgeAttribute(self, title, defaultValue, type="integer", mode="slice", force_id=""):
...
#             if self._edgesAttributes[id]["mode"]=="slice" and ( not start=="" or not end=="") :
#                 raise Exception("attribute "+str(id)+" is slice you can't specify start or end dates. Declare Attribute as dynamic")
#             return 1
#         else :
#             raise Exception("attribute id unknown. Add Attribute to graph first")
    ...
    def getXML(self):
        ...
        graphXML = etree.Element("graph", defaultedgetype=self.type, mode=self.mode, timestamp=self.timestamp, timerepresentation=self.timerepresentation)
    ...
    def importXML(graph_xml, gexf_obj):
        ...
        for attr in graph_xml.attrib:
            ...
            if attr == "timerepresentation":
                timerepresentation = graph_xml.attrib[attr]
            if attr == "timestamp":
                timestamp = graph_xml.attrib[attr]
        # create and attache the graph object to the Gexf object
        graph_obj = gexf_obj.addGraph(type=type, mode=mode,  timestamp=timestamp)
    ...
    def print_stat(self):
        print(self.timerepresentation + " " + self.type + " " + self.mode + " " + self.start + " " + self.end)
...
class Attributes(dict):
    """
        ...
        {
         "node" :
            { "id1" : {"id":"id1","title":"age","type":"integer","defaultValue":50,"mode":"slice"}, },
         "edge" :
    ...
    def __init__(self):
        ...
        self.mode_choices = ["slice", "dynamic"]
        for attClass in self.attClass_choices:
            self[attClass] = {}

    def declareAttribute(self, attClass, type, defaultValue, title="", mode="slice", id=None):
        """
    ...
    def makeAttributeInstance(self, attClass, id=None, value=None, start=None, end=None, startopen=False, endopen=False):
        ...
        if attClass in self.attClass_choices:
            if id in self[attClass].keys():
                ...
                # start & end will be discarded if the mode is set to slice
                    if start:
                        att["start"] = start
                    if startopen:
...
class Node:
        ...
        for child in nodes_xml:
            ...
            if tag == "node":
                ...
                for attr in node_xml.attrib:
                    ...
                    if attr == "timerepresentation":
                        timerepresentation = node_xml.attrib[attr]
                    if attr == "start":
    ...
    def __str__(self):
        return self.timerepresentation


class Edge:

    def __init__(self, graph, id, source, target, weight="", start="", end="", timerepresentation="", r="", g="", b="", spells=[], startopen=False, endopen=False):
    ...
    def getXML(self):
        # return lxml etree element
        try:
            edgeXML = etree.Element("edge", source=str(self._source), target=str(self._target))
            if not self.start == "":
            ...
            if not self.timerepresentation == "":
                edgeXML.set("timerepresentation", self.timerepresentation)
            ...
            if tag == "edge":
                ...
                for attr in edge_xml.attrib:
                    ...
                    if attr == "endopen":
                        ...
                        endopen = True
                    if attr == "timerepresentation":
                        timerepresentation = edge_xml.attrib[attr]
                ...
                edge_obj = graph_obj.addEdge(id=id, source=source, target=target, weight=weight, start=start, end=end, startopen=startopen, endopen=endopen, timerepresentation=timerepresentation, r=r, g=g, b=b, spells=spells)
...
# deprecated : import XML codes are now included to the Gexf, Graph, Attribute, Node, Edge classes
    ...
    def extract_graph_obj(self, graph_xml):
        ...
        self.graph_obj = self.gexf_obj.addGraph(type=type, mode=mode,  timestamp=timestamp)
    ...
    def extract_edge(self, edge_xml):
        ...
        self.edge_obj = self.graph_obj.addEdge(id=id, source=source, target=target, weight=weight, start=start, end=end, startopen=startopen, endopen=endopen, timerepresentation=timerepresentation, r=r, g=g, b=b, spells=spells)
